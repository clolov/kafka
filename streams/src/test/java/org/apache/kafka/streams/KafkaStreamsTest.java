...
public class KafkaStreamsTest {
    ...
    public void testShouldTransitToNotRunningIfCloseRightAfterCreated() {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            Assertions.assertEquals(KafkaStreams.State.NOT_RUNNING, streams.state());
        }
    }
    ...
    public void stateShouldTransitToRunningIfNonDeadThreadsBackToRunning() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            Assertions.assertEquals(0, streamsStateListener.numChanges);
            Assertions.assertEquals(KafkaStreams.State.CREATED, streams.state());
            ...
            Assertions.assertEquals(KafkaStreams.State.RUNNING, streams.state());
            waitForCondition(
                () -> streamsStateListener.numChanges == 2,
                "Streams never started.");
            Assertions.assertEquals(KafkaStreams.State.RUNNING, streams.state());
            ...
            Assertions.assertEquals(3, streamsStateListener.numChanges);
            Assertions.assertEquals(KafkaStreams.State.REBALANCING, streams.state());
            ...
            Assertions.assertEquals(4, streamsStateListener.numChanges);
            Assertions.assertEquals(KafkaStreams.State.RUNNING, streams.state());
    ...
    public void shouldCleanupResourcesOnCloseWithoutPreviousStart() throws Exception {
        ...
        Assertions.assertTrue(supplier.consumer.closed());
        Assertions.assertTrue(supplier.restoreConsumer.closed());
        for (final MockProducer<byte[], byte[]> p : supplier.producers) {
            Assertions.assertTrue(p.closed());
        }
    }
    ...
    public void testStateThreadClose() throws Exception {
        ...
        try (final KafkaStreams streams = new KafkaStreams(builder.build(), props, supplier, time)) {
            Assertions.assertEquals(NUM_THREADS, streams.threads.size());
            Assertions.assertEquals(streams.state(), KafkaStreams.State.CREATED);
            ...
            Assertions.assertNull(streams.globalStreamThread);
        }
    }
    ...
    public void testInitializesAndDestroysMetricsReporters() {
        ...
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            Assertions.assertTrue(initDiff == 1, "some reporters including MockMetricsReporter should be initialized by calling on construction");

            streams.start();
            final int oldCloseCount = MockMetricsReporter.CLOSE_COUNT.get();
            streams.close();
            Assertions.assertEquals(streams.state(), KafkaStreams.State.NOT_RUNNING);
            Assertions.assertEquals(oldCloseCount + initDiff, MockMetricsReporter.CLOSE_COUNT.get());
        }
    }
    ...
    public void testCloseIsIdempotent() {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            Assertions.assertEquals(
                    closeCount, MockMetricsReporter.CLOSE_COUNT.get(), "subsequent close() calls should do nothing");
        }
    }
    ...
    public void shouldThrowOnCleanupWhileRunning() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            try {
                ...
                Assertions.assertEquals("Cannot clean up while running.", expected.getMessage());
            }
        }
    }
    ...
    public void shouldThrowOnCleanupWhileShuttingDown() throws InterruptedException {
        ...
        Assertions.assertThrows(IllegalStateException.class, streams::cleanUp);
        assertThat(streams.state() == State.PENDING_SHUTDOWN, equalTo(true));
    }
    ...
    public void shouldNotGetAllTasksWhenNotRunning() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            Assertions.assertThrows(StreamsNotStartedException.class, streams::metadataForAllStreamsClients);
            streams.start();
            ...
            Assertions.assertThrows(IllegalStateException.class, streams::metadataForAllStreamsClients);
        }
    }
    ...
    public void shouldNotGetQueryMetadataWithSerializerWhenNotRunningOrRebalancing() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            Assertions.assertThrows(StreamsNotStartedException.class, () -> streams.queryMetadataForKey("store", "key", Serdes.String().serializer()));
            streams.start();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.RUNNING, DEFAULT_DURATION);
            streams.close();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.NOT_RUNNING, DEFAULT_DURATION);
            Assertions.assertThrows(IllegalStateException.class, () -> streams.queryMetadataForKey("store", "key", Serdes.String().serializer()));
        }
    }
    ...
    public void shouldGetQueryMetadataWithSerializerWhenRunningOrRebalancing() {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            streams.start();
            Assertions.assertEquals(KeyQueryMetadata.NOT_AVAILABLE, streams.queryMetadataForKey("store", "key", Serdes.String().serializer()));
        }
    }
    ...
    public void shouldNotGetQueryMetadataWithPartitionerWhenNotRunningOrRebalancing() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            Assertions.assertThrows(StreamsNotStartedException.class, () -> streams.queryMetadataForKey("store", "key", (topic, key, value, numPartitions) -> 0));
            streams.start();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.RUNNING, DEFAULT_DURATION);
            streams.close();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.NOT_RUNNING, DEFAULT_DURATION);
            Assertions.assertThrows(IllegalStateException.class, () -> streams.queryMetadataForKey("store", "key", (topic, key, value, numPartitions) -> 0));
        }
    }
    ...
    public void shouldThrowUnknownStateStoreExceptionWhenStoreNotExist() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            streams.start();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.RUNNING, DEFAULT_DURATION);
            Assertions.assertThrows(UnknownStateStoreException.class, () -> streams.store(StoreQueryParameters.fromNameAndType("unknown-store", keyValueStore())));
        }
    }
    ...
    public void shouldNotGetStoreWhenWhenNotRunningOrRebalancing() throws InterruptedException {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            Assertions.assertThrows(StreamsNotStartedException.class, () -> streams.store(StoreQueryParameters.fromNameAndType("store", keyValueStore())));
            streams.start();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.RUNNING, DEFAULT_DURATION);
            streams.close();
            waitForApplicationState(Collections.singletonList(streams), KafkaStreams.State.NOT_RUNNING, DEFAULT_DURATION);
            Assertions.assertThrows(IllegalStateException.class, () -> streams.store(StoreQueryParameters.fromNameAndType("store", keyValueStore())));
        }
    }
    ...
    public void shouldReturnEmptyLocalStorePartitionLags() {
        ...
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, mockClientSupplier, time)) {
            streams.start();
            Assertions.assertEquals(0, streams.allLocalStorePartitionLags().size());
        }
    }
    ...
    public void shouldReturnFalseOnCloseWhenThreadsHaventTerminated() {
        ...
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier)) {
            Assertions.assertFalse(streams.close(Duration.ofMillis(10L)));
        }
    }
    ...
    public void shouldNotBlockInCloseForZeroDuration() {
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier, time)) {
            ...
            Assertions.assertFalse(streams.close(Duration.ZERO));
        }
    }
    ...
    public void shouldReturnFalseOnCloseWithCloseOptionWithLeaveGroupFalseWhenThreadsHaventTerminated() {
        ...
        try (final KafkaStreams streams = new KafkaStreams(getBuilderWithSource().build(), props, supplier)) {
            Assertions.assertFalse(streams.close(closeOptions));
        }
    }
    ...
    @Test
    public void statelessTopologyShouldNotCreateStateDirectory(final TestInfo testInfo) throws Exception {
    ...
    @Test
    public void inMemoryStatefulTopologyShouldNotCreateStateDirectory(final TestInfo testInfo) throws Exception {
        final String safeTestName = safeUniqueTestName(getClass(), testInfo);
        final String inputTopic = safeTestName + "-input";
        final String outputTopic = safeTestName + "-output";
    ...
    @Test
    public void statefulTopologyShouldCreateStateDirectory(final TestInfo testInfo) throws Exception {
        final String safeTestName = safeUniqueTestName(getClass(), testInfo);
        final String inputTopic = safeTestName + "-input";
        final String outputTopic = safeTestName + "-output";
        final String globalTopicName = safeTestName + "-global";
